...
public:
    ...
    SampleRep(int sample,int rep):sampleNum(sample), repNum(rep){};
    
    bool operator <(const SampleRep & other) const {
        if(repNum<other.repNum){return true;};
        if(repNum==other.repNum){
            return sampleNum<other.sampleNum;
        }
        return false;
    };
};
